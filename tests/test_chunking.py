...
from healthcompass.ingestion import load_document, clean_page
...
def test_both_strategies_work_on_same_document(vaccination_guidance):
    ...
    fixed_chunks = chunk_document(vaccination_guidance, strategy="fixed", chunk_size=500, overlap=100)
...
def test_chunk_ids_are_sequential():
    ...
    assert [chunk.chunk_id for chunk in paragraph_chunks_list] == list(range(len(paragraph_chunks_list)))
