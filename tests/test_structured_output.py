...
class TestParseStructuredResponse:
    ...
    def test_missing_closing_brace_recovery(self):
        ...
        assert result.data == {"answer": "Use current guidance", "source": "Outbreak Guideline"}
...
class TestFormatValidationError:
    ...
    def test_format_error(self):
        ...
        result = StructuredOutputResult(
            success=False, error="Test error message", recovery_attempted=False
        )
        formatted = format_validation_error(result)
    ...
    def test_format_error_with_recovery(self):
        ...
        result = StructuredOutputResult(
            success=False, error="Test error message", recovery_attempted=True
        )
        formatted = format_validation_error(result)
...
class TestStructuredOutputResult:
    ...
    def test_to_dict_success(self):
        ...
        result = StructuredOutputResult(
            success=True, data={"answer": "test"}, raw_input="test input"
        )
    ...
    def test_to_dict_failure(self):
        ...
        result = StructuredOutputResult(
            success=False, error="Test error", raw_input="test input", recovery_attempted=True
        )
        result_dict = result.to_dict()
...
class TestMalformedThenRecovered:
    ...
    def test_malformed_then_recovered_case(self):
        ...
        assert initial_parse.success
        assert initial_parse.recovery_attempted
...
@pytest.mark.parametrize(
    "answer", ["Literal { and [ characters", 'Quoted "text" and \\ escapes', "ending ,}"]
)
def test_valid_json_string_delimiters_are_not_repaired(answer):
    import json

    raw = json.dumps({"answer": answer, "source": "Guide"})
...
@pytest.mark.parametrize("extra", [{"version": 12}, {"effective_date": None}, {"unknown": "value"}])
def test_declared_optional_schema_is_enforced(extra):
    import json

    assert not parse_and_validate(json.dumps({"answer": "ok", "source": "Guide", **extra})).success


def test_failed_api_calls_produce_reports_and_failure_exit(monkeypatch, tmp_path):
    import json
    ...
    from experiments.structured_output import structured_output as module

    monkeypatch.setenv("OPENAI_API_KEY", "test-only")
    monkeypatch.setenv("CHAT_MODEL", "test-model")
    factory = MagicMock()
    factory.return_value.__enter__.return_value.chat.completions.create.side_effect = (
    ...
    assert (
        module.main(["--repetitions", "1", "--output", str(output), "--markdown", str(markdown)])
        == 1
    )
    report = json.loads(output.read_text())
    assert report["total_api_calls"] == 2
    assert report["successful_api_calls"] == 0
    assert "Validation Success:** False" in markdown.read_text()
    factory.reset_mock()
    assert module.main(["--output", str(output), "--markdown", str(markdown)]) == 1
    factory.assert_not_called()


def test_truncated_json_is_not_repaired_into_success():
    from types import SimpleNamespace
    from unittest.mock import Mock

    from experiments.structured_output.structured_output import StructuredOutputRunner
    ...
    client.chat.completions.create.return_value = SimpleNamespace(
        choices=[
            SimpleNamespace(
                ...
                message=SimpleNamespace(content='{"answer":"ok","source":"Guide"'),
            )
        ],
        usage=None,
    )
    result = StructuredOutputRunner(client, "test").request_structured_output()
