...
def save_comparison_report(
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        ...
        if paragraph_stats.max_chunk_size > 1000:
            ...
            recommendation = "Fixed-size + overlap"
            reason = "The document contains very short paragraphs (minimum: {} chars) that would produce tiny chunks lacking context for effective retrieval.".format(paragraph_stats.min_chunk_size)
        elif paragraph_stats.avg_chunk_size > 800:
            recommendation = "Fixed-size + overlap"
            reason = "Average paragraph size is large ({} chars), which could reduce retrieval precision in vector search.".format(int(paragraph_stats.avg_chunk_size))
        else:
            recommendation = "Paragraph-based"
            reason = "The document has well-structured paragraphs of reasonable size (avg: {} chars, max: {} chars), preserving semantic coherence for better retrieval quality.".format(int(paragraph_stats.avg_chunk_size), paragraph_stats.max_chunk_size)
        ...
        f.write("- The context window is the maximum amount of text/tokens the model can process in one request\n")
        f.write("- Chunks should be small enough that multiple retrieved chunks plus the user's question and system instructions fit comfortably\n")
        ...
        f.write("- Chunk size should therefore leave room for multiple relevant chunks and the generated answer\n\n")
        f.write("For HealthCompass, with typical context windows of 4K-8K tokens, chunk sizes of 500-1000 characters balance context preservation with retrieval precision.")
