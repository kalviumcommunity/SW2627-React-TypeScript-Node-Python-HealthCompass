...
JSON_SCHEMA = {
    "type": "object",
    "properties": {
        "answer": {"type": "string", "description": "A concise factual answer to the question"},
        "source": {
            "type": "string",
            "description": "The name of the source document or repository",
        },
        "version": {"type": "string", "description": "Document version if applicable"},
        "effective_date": {
            "type": "string",
...
class StructuredOutputRunner:
    ...
    def request_structured_output(
        self, use_json_mode: bool = True, temperature: float = 0.2, max_tokens: int = 300
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if result["finish_reason"] != "stop":
                result["error"] = "Response was not completed normally; no JSON recovery attempted"
                return result
            # Parse and validate
...
def save_results(results: list[Dict[str, Any]], output_path: Path) -> None:
    ...
    report = {
        ...
        "successful_api_calls": sum(bool(r.get("api_call_success")) for r in results),
        "results": results,
    }
...
def generate_markdown_report(results: list[Dict[str, Any]], output_path: Path) -> None:
    ...
    with output_path.open("x", encoding="utf-8") as f:
        ...
        json_mode_success = sum(
            1 for r in json_mode_results if (r.get("validation_result") or {}).get("success", False)
        )
        regular_success = sum(
            1 for r in regular_results if (r.get("validation_result") or {}).get("success", False)
        )
        ...
        for i, result in enumerate(json_mode_results, 1):
            ...
            f.write(
                f"**Validation Success:** {(result.get('validation_result') or {}).get('success', False)}\n"
            )
            ...
            if (result.get("validation_result") or {}).get("recovery_attempted"):
        ...
        f.write(
            "Counts describe observed validation outcomes only. Failed calls are not valid outputs.\n"
        )
        f.write(
            "This experiment does not establish correctness, source grounding, or optimal settings.\n"
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path("outputs/structured-results.json"))
    parser.add_argument("--markdown", type=Path, default=Path("outputs/structured-results.md"))
    ...
    try:
        ...
        if not args.dry_run and (args.output.exists() or args.markdown.exists()):
            raise ValueError("Output already exists; choose new report paths")
        load_dotenv()
        ...
        if not args.dry_run:
            ...
            if any(not os.getenv(key, "").strip() for key in required):
                raise ValueError(
                    "Configure OPENAI_API_KEY and CHAT_MODEL before running experiments"
                )
        ...
        return (
            0
            if all((r.get("validation_result") or {}).get("success", False) for r in results)
            else 1
        )
