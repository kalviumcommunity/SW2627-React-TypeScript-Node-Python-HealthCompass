...
from healthcompass.ingestion import ingest_corpus, DocumentLoadError, load_document
    ...
    try:
        ...
        if result.loaded:
            ...
            for page in result.loaded:
                if page.filename not in by_filename:
                    by_filename[page.filename] = []
                by_filename[page.filename].append(page)
            
            for filename, pages in by_filename.items():
                text_length = sum(len(page.text) for page in pages)
                text_preview = pages[0].text[:60].replace("\n", " ") if text_length > 60 else pages[0].text.replace("\n", " ")
                print(f"OK {filename}: {text_length} chars | '{text_preview}...'")
...
def main():
    parser = argparse.ArgumentParser(
        description="Document intake demo for multi-format loading"
    )
    parser.add_argument(
        "path",
        help="Path to document file or directory for corpus ingestion"
    )
    parser.add_argument(
        "--corpus",
        action="store_true",
        help="Treat path as directory for corpus ingestion"
    )
    ...
    if args.corpus:
        ...
        if path.is_dir():
            print(f"Error: Path is a directory. Use --corpus flag for directory ingestion.", file=sys.stderr)
            return 1
